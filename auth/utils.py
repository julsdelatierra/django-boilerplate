...
TWITTERAUTH_KEY = getattr(settings, 'TWITTERAUTH_KEY', settings.TWITTERAUTH_KEY)
TWITTERAUTH_SECRET = getattr(settings, 'TWITTERAUTH_SECRET', settings.TWITTERAUTH_SECRET)
...
def connection():
	try: return connection._connection
	    ...
	    connection._connection = urllib2
...
def oauth_response(req):
    return connection().urlopen(req.to_url()).read()
